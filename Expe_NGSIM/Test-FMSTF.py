...
'------------------------------------------------------------------------------------'
model=FMSTF().cuda()
model.load_state_dict(torch.load('../PretrainParams/NGSIM/FMSTF_50.pth'))
loss_mse=nn.MSELoss()
...
for t in tqdm(range(int(len(dataset_test[0])/args.batch_size))):
    ...
    his_train, obs_mask = Rand_Mask(copy.deepcopy(his), copy.deepcopy(scale_mask), point,rate_low=0.0, rate_high=0.3)  # 掩码
    # flops, params = profile(model, inputs=(his_train, obs_mask, social_mask, point, ))
    # print('params:',params/1e6,'flops:',flops/1e6/args.batch_size)
    # input()
    fut_pre = model(his_train, obs_mask, social_mask, point)  # 预测解码
