...
'------------------------------------------------------------------------------------'
model=FMSTF().cuda()
model.load_state_dict(torch.load('../PretrainParams/IArgoverse/FMSTF_176.pth'))


'------------------------------------------------------------------------------------------------'
pre_len=1
preds=torch.zeros((1,10*pre_len,2)).cuda()
gts=torch.zeros((1,10*pre_len,2)).cuda()
model.eval()
with torch.no_grad():
    for t in tqdm(range(int(len(dataset_test[0])/args.batch_size))):
       # (1) Load data and convert it to tensor
        ...
        preds = torch.cat((preds, fut_pre[:,0:10*pre_len,:]), 0)
        gts = torch.cat((gts, fut_label[:,0:10*pre_len,:]), 0)
